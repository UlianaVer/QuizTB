...
async def process_age(message: types.Message, state: FSMContext):
   ...
   markup.add("2017", "2018", "2019", "2020", "2039")

   await message.reply("В каком году ты поступил?", reply_markup=markup)


@dp.message_handler(lambda message: message.text not in ["2017", "2018", "2019", "2020", "2039"], state=Form.startedu)
async def process_gender_invalid(message: types.Message):
   return await message.reply("Ответ неверный. Выбери нужный вариант на клавиатуре.")
   ...
   async with state.proxy() as data:
       ...
       await bot.send_message(
           ...
           md.text(
               ...
               md.text('Начало обучения в:', data['startedu']),
               sep='\n',
           ),
           reply_markup=markup,
